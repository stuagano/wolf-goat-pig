...
class TestCompleteHoleFlow:
        ...
        while shots_played < 20:  # Safety limit
            ...
            if partnership_actions and not partnership_requested:
                ...
                response = client.post(
                    ...
                    json={"action_type": "REQUEST_PARTNERSHIP", "payload": partnership_actions[0]["payload"]},
                )
                assert response.status_code == 200
                ...
                # Accept partnership
                response = client.post(
                    f"/wgp/{game_id}/action", json={"action_type": "RESPOND_PARTNERSHIP", "payload": {"accepted": True}}
                )
                assert response.status_code == 200
                break
    ...
    def test_hole_with_solo_captain(self, client, initialized_game):
        ...
        for _ in range(10):
            ...
            if solo_actions:
                # Go solo
                response = client.post(f"/wgp/{game_id}/action", json={"action_type": "DECLARE_SOLO"})
                assert response.status_code == 200
    ...
    def test_hole_with_double_wager(self, client, initialized_game):
        ...
        for _ in range(15):
            ...
            if double_actions and not double_offered:
                ...
                response = client.post(
                    f"/wgp/{game_id}/action",
                    json={"action_type": "OFFER_DOUBLE", "payload": double_actions[0]["payload"]},
                )
                assert response.status_code == 200
                double_offered = True
                ...
                # Accept double
                response = client.post(
                    f"/wgp/{game_id}/action", json={"action_type": "ACCEPT_DOUBLE", "payload": {"accepted": True}}
                )
                assert response.status_code == 200
            ...
            while shots_in_hole < 30 and not hole_complete:  # Safety limit
                ...
                if response.status_code != 200:
                    ...
                    if available_actions:
                        ...
                        response = client.post(
                            f"/wgp/{game_id}/action",
                            json={"action_type": action["action_type"], "payload": action.get("payload", {})},
                        )
                ...
                if score_actions:
                    ...
                    response = client.post(
                        f"/wgp/{game_id}/action",
                        json={"action_type": "ENTER_HOLE_SCORES", "payload": {"scores": scores}},
                    )
                    assert response.status_code == 200

                    # Advance to next hole
                    response = client.post(f"/wgp/{game_id}/action", json={"action_type": "ADVANCE_HOLE"})
                    assert response.status_code == 200
...
class TestSpecialScenarios:
    ...
    def test_hole_18_big_dick(self, client):
        ...
        response = client.post(
            f"/wgp/{game_id}/action", json={"action_type": "OFFER_BIG_DICK", "payload": {"player_id": "test-player-1"}}
        )
    ...
    def test_analytics_retrieval(self, client, initialized_game):
        ...
        for _ in range(5):
            client.post(f"/wgp/{game_id}/action", json={"action_type": "PLAY_SHOT"})

        # Get analytics
        response = client.post(f"/wgp/{game_id}/action", json={"action_type": "GET_ADVANCED_ANALYTICS"})
        assert response.status_code == 200
    ...
    def test_post_hole_analysis(self, client, initialized_game):
        ...
        response = client.post(
            f"/wgp/{game_id}/action", json={"action_type": "GET_POST_HOLE_ANALYSIS", "payload": {"hole_number": 1}}
        )
        assert response.status_code == 200
...
class TestErrorRecovery:
    ...
    def test_invalid_partnership_request(self, client, initialized_game):
        ...
        response = client.post(
            f"/wgp/{game_id}/action",
            json={"action_type": "REQUEST_PARTNERSHIP", "payload": {"target_player_name": "NonexistentPlayer"}},
        )
