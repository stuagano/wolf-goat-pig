...
def valid_course_data():
    ...
    return {"course_rating": 72.5, "slope_rating": 113, "par": 72}
...
class TestHandicapValidator:
    ...
    def test_calculate_net_score_no_strokes(self):
        ...
        net = HandicapValidator.calculate_net_score(gross_score=4, strokes_received=0)
        assert net == 4

    def test_calculate_net_score_one_stroke(self):
        """Test net score calculation with one stroke received."""
        net = HandicapValidator.calculate_net_score(gross_score=5, strokes_received=1)
        assert net == 4

    def test_calculate_net_score_multiple_strokes(self):
        """Test net score calculation with multiple strokes received."""
        net = HandicapValidator.calculate_net_score(gross_score=7, strokes_received=2)
        assert net == 5
    ...
    def test_calculate_net_score_without_validation(self):
        ...
        net = HandicapValidator.calculate_net_score(gross_score=5, strokes_received=1, validate=False)
        assert net == 4
    ...
    def test_validate_course_rating_valid(self, valid_course_data):
        ...
        HandicapValidator.validate_course_rating(valid_course_data["course_rating"], valid_course_data["slope_rating"])
    ...
    def test_calculate_course_handicap_difficult_course(self):
        ...
        course_handicap = HandicapValidator.calculate_course_handicap(
            handicap_index=10.0, slope_rating=140, course_rating=74.0, par=72
        )
    ...
    def test_calculate_course_handicap_easy_course(self):
        ...
        course_handicap = HandicapValidator.calculate_course_handicap(
            handicap_index=10.0, slope_rating=90, course_rating=70.0, par=72
        )
    ...
    def test_calculate_course_handicap_scratch_golfer(self):
        ...
        course_handicap = HandicapValidator.calculate_course_handicap(
            handicap_index=0.0, slope_rating=113, course_rating=72.0, par=72
        )
    ...
    def test_validate_stroke_allocation_valid(self, valid_handicaps, stroke_indexes):
        ...
        HandicapValidator.validate_stroke_allocation(
            players_handicaps=valid_handicaps[:4], hole_stroke_indexes=stroke_indexes
        )
    ...
    def test_validate_stroke_allocation_duplicate_indexes(self):
        ...
        with pytest.raises(HandicapValidationError) as exc_info:
            HandicapValidator.validate_stroke_allocation(
                players_handicaps=[10.0], hole_stroke_indexes=duplicate_indexes
            )
    ...
    def test_validate_stroke_allocation_missing_indexes(self):
        ...
        with pytest.raises(HandicapValidationError) as exc_info:
            HandicapValidator.validate_stroke_allocation(players_handicaps=[10.0], hole_stroke_indexes=missing_indexes)
    ...
    def test_validate_team_handicaps_balanced(self):
        ...
        result = HandicapValidator.validate_team_handicaps(team1_handicaps=[10.0, 15.0], team2_handicaps=[12.0, 13.0])
    ...
    def test_validate_team_handicaps_empty_team(self):
        ...
        with pytest.raises(HandicapValidationError) as exc_info:
            HandicapValidator.validate_team_handicaps(team1_handicaps=[], team2_handicaps=[12.0, 13.0])
...
class TestBettingValidator:
    ...
    def test_validate_double_allowed(self):
        ...
        BettingValidator.validate_double(already_doubled=False, wagering_closed=False, partnership_formed=True)

    def test_validate_double_already_doubled(self):
        """Test double validation rejects when already doubled."""
        with pytest.raises(BettingValidationError) as exc_info:
            BettingValidator.validate_double(already_doubled=True, wagering_closed=False, partnership_formed=True)
    ...
    def test_validate_double_wagering_closed(self):
        ...
        with pytest.raises(BettingValidationError) as exc_info:
            BettingValidator.validate_double(already_doubled=False, wagering_closed=True, partnership_formed=True)
    ...
    def test_validate_double_no_partnership(self):
        ...
        with pytest.raises(BettingValidationError) as exc_info:
            BettingValidator.validate_double(already_doubled=False, wagering_closed=False, partnership_formed=False)
    ...
    def test_validate_duncan_allowed(self):
        ...
        BettingValidator.validate_duncan(is_captain=True, partnership_formed=False, tee_shots_complete=False)

    def test_validate_duncan_not_captain(self):
        """Test Duncan validation rejects non-captain."""
        with pytest.raises(BettingValidationError) as exc_info:
            BettingValidator.validate_duncan(is_captain=False, partnership_formed=False, tee_shots_complete=False)
    ...
    def test_validate_duncan_partnership_formed(self):
        ...
        with pytest.raises(BettingValidationError) as exc_info:
            BettingValidator.validate_duncan(is_captain=True, partnership_formed=True, tee_shots_complete=False)
    ...
    def test_validate_duncan_after_tee_shots(self):
        ...
        with pytest.raises(BettingValidationError) as exc_info:
            BettingValidator.validate_duncan(is_captain=True, partnership_formed=False, tee_shots_complete=True)
    ...
    def test_validate_carry_over_allowed(self):
        ...
        BettingValidator.validate_carry_over(hole_number=5, previous_hole_tied=True)

    def test_validate_carry_over_first_hole(self):
        """Test carry over validation rejects on first hole."""
        with pytest.raises(BettingValidationError) as exc_info:
            BettingValidator.validate_carry_over(hole_number=1, previous_hole_tied=True)
    ...
    def test_validate_carry_over_not_tied(self):
        ...
        with pytest.raises(BettingValidationError) as exc_info:
            BettingValidator.validate_carry_over(hole_number=5, previous_hole_tied=False)
    ...
    def test_calculate_wager_multiplier_base(self):
        ...
        multiplier = BettingValidator.calculate_wager_multiplier(doubled=False, carry_over=False, duncan=False)
        assert multiplier == 1

    def test_calculate_wager_multiplier_doubled(self):
        """Test wager multiplier calculation with double."""
        multiplier = BettingValidator.calculate_wager_multiplier(doubled=True, carry_over=False, duncan=False)
        assert multiplier == 2

    def test_calculate_wager_multiplier_carry_over(self):
        """Test wager multiplier calculation with carry over."""
        multiplier = BettingValidator.calculate_wager_multiplier(doubled=False, carry_over=True, duncan=False)
        assert multiplier == 2

    def test_calculate_wager_multiplier_duncan(self):
        """Test wager multiplier calculation with Duncan."""
        multiplier = BettingValidator.calculate_wager_multiplier(doubled=False, carry_over=False, duncan=True)
        assert multiplier == 2

    def test_calculate_wager_multiplier_all_modifiers(self):
        """Test wager multiplier calculation with all modifiers."""
        multiplier = BettingValidator.calculate_wager_multiplier(doubled=True, carry_over=True, duncan=True)
    ...
    def test_calculate_total_wager_base(self):
        ...
        total = BettingValidator.calculate_total_wager(base_wager=1, multiplier=1)
        assert total == 1

    def test_calculate_total_wager_doubled(self):
        """Test total wager calculation with double."""
        total = BettingValidator.calculate_total_wager(base_wager=1, multiplier=2)
        assert total == 2

    def test_calculate_total_wager_high_stakes(self):
        """Test total wager calculation with high stakes."""
        total = BettingValidator.calculate_total_wager(base_wager=5, multiplier=8)
        assert total == 40
...
class TestGameStateValidator:
    ...
    def test_validate_player_action_valid(self):
        ...
        GameStateValidator.validate_player_action(
            player_id="p1", action="shoot", current_player="p1", game_phase="PLAYING"
        )
    ...
    def test_validate_player_action_wrong_phase(self):
        ...
        with pytest.raises(GameStateValidationError) as exc_info:
            GameStateValidator.validate_player_action(
                player_id="p1", action="shoot", current_player="p1", game_phase="SETUP"
            )
    ...
    def test_validate_partnership_formation_valid(self):
        ...
        GameStateValidator.validate_partnership_formation(captain_id="p1", partner_id="p2", tee_shots_complete=False)
    ...
    def test_validate_partnership_formation_self(self):
        ...
        with pytest.raises(GameStateValidationError) as exc_info:
            GameStateValidator.validate_partnership_formation(
                captain_id="p1", partner_id="p1", tee_shots_complete=False
            )
    ...
    def test_validate_partnership_formation_after_deadline(self):
        ...
        with pytest.raises(GameStateValidationError) as exc_info:
            GameStateValidator.validate_partnership_formation(captain_id="p1", partner_id="p2", tee_shots_complete=True)
    ...
    def test_validate_shot_execution_valid(self):
        ...
        GameStateValidator.validate_shot_execution(player_id="p1", hole_complete=False, player_holed=False)

    def test_validate_shot_execution_hole_complete(self):
        """Test shot execution validation rejects when hole complete."""
        with pytest.raises(GameStateValidationError) as exc_info:
            GameStateValidator.validate_shot_execution(player_id="p1", hole_complete=True, player_holed=False)
    ...
    def test_validate_shot_execution_player_holed(self):
        ...
        with pytest.raises(GameStateValidationError) as exc_info:
            GameStateValidator.validate_shot_execution(player_id="p1", hole_complete=False, player_holed=True)
    ...
    def test_validate_game_start_valid(self):
        ...
        GameStateValidator.validate_game_start(player_count=4, course_selected=True, all_players_ready=True)

    def test_validate_game_start_no_course(self):
        """Test game start validation rejects without course."""
        with pytest.raises(GameStateValidationError) as exc_info:
            GameStateValidator.validate_game_start(player_count=4, course_selected=False, all_players_ready=True)
    ...
    def test_validate_game_start_players_not_ready(self):
        ...
        with pytest.raises(GameStateValidationError) as exc_info:
            GameStateValidator.validate_game_start(player_count=4, course_selected=True, all_players_ready=False)
    ...
    def test_validate_game_start_invalid_player_count(self):
        ...
        with pytest.raises(GameStateValidationError) as exc_info:
            GameStateValidator.validate_game_start(player_count=1, course_selected=True, all_players_ready=True)
    ...
    def test_validate_hole_completion_valid(self):
        ...
        GameStateValidator.validate_hole_completion(players_holed=["p1", "p2", "p3", "p4"], total_players=4)

    def test_validate_hole_completion_incomplete(self):
        """Test hole completion validation rejects when players remain."""
        with pytest.raises(GameStateValidationError) as exc_info:
            GameStateValidator.validate_hole_completion(players_holed=["p1", "p2"], total_players=4)
...
class TestValidationExceptions:
    ...
    def test_validation_error_to_dict(self):
        ...
        error = ValidationError("Test error", field="test_field", details={"value": 60.0})
...
class TestValidatorIntegration:
    ...
    def test_full_handicap_workflow(self):
        ...
        strokes = HandicapValidator.calculate_strokes_received_with_creecher(
            course_handicap=course_handicap, stroke_index=stroke_index
        )

        # 5. Calculate net score
        gross_score = 5
        net_score = HandicapValidator.calculate_net_score(gross_score=gross_score, strokes_received=strokes)
    ...
    def test_full_betting_workflow(self):
        ...
        total = BettingValidator.calculate_total_wager(base_wager=base_wager, multiplier=multiplier)
    ...
    def test_cross_validator_scenario(self):
        ...
        GameStateValidator.validate_game_start(player_count=len(players), course_selected=True, all_players_ready=True)
