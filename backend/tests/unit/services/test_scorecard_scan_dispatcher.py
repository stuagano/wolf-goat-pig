...
@pytest.mark.asyncio
async def test_dispatch_defaults_to_agent(monkeypatch):
    ...
    mock_agent.assert_awaited_once_with(b"img", "image/jpeg", None)


@pytest.mark.asyncio
async def test_dispatch_groq_provider_when_explicit(monkeypatch):
    monkeypatch.setenv("SCORECARD_VISION_PROVIDER", "groq")
...
@pytest.mark.asyncio
async def test_dispatch_agent_provider_explicit(monkeypatch):
