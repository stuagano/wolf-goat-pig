...
class TestTeamPairingsWithRotations:
    ...
    def test_create_rotations(self):
        ...
        rotations = TeamFormationService.create_team_pairings_with_rotations(players, num_rotations=3, seed=42)
...
class TestRandomNumberGeneration:
    ...
    def test_no_seed_different_results(self):
        ...
        teams_list = [TeamFormationService.generate_random_teams(players) for _ in range(5)]

        # At least some should be different (highly likely)
        player_orders = [[p["player_id"] for team in teams for p in team["players"]] for teams in teams_list]
