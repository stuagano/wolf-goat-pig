...
class TestGameCompletionPersistence:
    ...
    def test_game_completion_persistence(self):
        ...
        # Mock database session before creating game instance
        with patch('backend.app.mixins.persistence_mixin.SessionLocal') as mock_session_cls, \
             patch.object(WolfGoatPigGame, '_load_from_db'):
            mock_db_session = MagicMock()
            mock_session_cls.return_value = mock_db_session

            # Initialize game with mocked database
            sim = WolfGoatPigGame(player_count=4, players=players)

            # Mock course manager to avoid attribute errors
            mock_course_manager = MagicMock()
            mock_course_manager.selected_course = "Test Course"
            sim.course_manager = mock_course_manager
            ...
            assert len(game_record_calls) >= 1, "Should have saved at least one GameRecord"

            # Get the last GameRecord saved (the completion record)
            saved_record = game_record_calls[-1][0][0]
