    ...
    try:
        ...
        for course in courses:
            ...
            for i, hole in enumerate(holes[:3]):  # Show first 3 holes
                print(
                    f"     Hole {hole['hole_number']}: Par {hole['par']}, {hole['yards']} yards, "
        ...
        for course in courses:
            ...
            print(
                f"    Difficulty: {'Easy' if difficulty_factor < 0.5 else 'Medium' if difficulty_factor < 1.0 else 'Hard'}"
            )
            print(
                f"    Total Length: {'Short' if course.total_yards < 6000 else 'Medium' if course.total_yards < 7500 else 'Long'}"
            )
            print()
