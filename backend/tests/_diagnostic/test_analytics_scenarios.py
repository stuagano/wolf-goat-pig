...
class AnalyticsScenarioTester:
    ...
    def test_partnership_success_scenario(self):
        ...
        response = self.session.post(
            ...
            json={"action_type": "REQUEST_PARTNERSHIP", "payload": {"requested_partner": "p2"}},
        )
    ...
    def test_aggressive_betting_scenario(self):
        ...
        response = self.session.post(
            f"{API_BASE}/wgp/{self.game_id}/action",
            json={"action_type": "CAPTAIN_GO_SOLO", "payload": {"use_duncan": True}},
        )

        # Try to offer double after good shot
        print("Attempting to offer double...")
        response = self.session.post(
            f"{API_BASE}/wgp/{self.game_id}/action", json={"action_type": "OFFER_DOUBLE", "payload": {}}
        )
    ...
    def test_poor_performance_scenario(self):
        ...
        response = self.session.post(
            f"{API_BASE}/wgp/{self.game_id}/action", json={"action_type": "INITIALIZE_GAME", "payload": players_data}
        )
    ...
    def _play_shot(self):
        """Play a single shot"""
        response = self.session.post(
            f"{API_BASE}/wgp/{self.game_id}/action", json={"action_type": "PLAY_SHOT", "payload": {}}
        )
        return response.status_code == 200
