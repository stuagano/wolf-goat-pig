        ...
        if success:
            ...
            try:
                ...
                if results.get("golfers"):
                    ...
                    logger.info(
                        f"   First result: {first_golfer.get('first_name')} {first_golfer.get('last_name')} - GHIN: {first_golfer.get('ghin')}"
                    )
            ...
            player_with_ghin = db.query(PlayerProfile).filter(PlayerProfile.ghin_id.isnot(None)).first()
            ...
            if player_with_ghin:
                ...
                if handicap_data:
                    logger.info("✅ Handicap synced successfully!")
                ...
                test_player = db.query(PlayerProfile).filter(PlayerProfile.name == "Stuart").first()
...
async def test_specific_ghin_lookup(ghin_id: str):
    ...
    try:
        ...
        if await ghin_service.initialize():
            ...
            if handicap_data:
                logger.info("✅ Found handicap data:")
