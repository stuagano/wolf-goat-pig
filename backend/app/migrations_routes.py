...
def verify_admin_key(x_admin_key: str = Header(...)) -> bool:
    ...
    if x_admin_key != expected_key:
        raise HTTPException(
            status_code=403,
            detail="Invalid admin key. Set X-Admin-Key header."
        )
    return True
...
def _add_tee_order_migration(db: Session) -> Dict[str, Any]:
    ...
    if 'game_players' not in inspector.get_table_names():
        return {
            "message": "game_players table does not exist",
            "changes": []
        }
    ...
    if 'tee_order' not in columns:
        ...
        # Add index
        db.execute(text(
            "CREATE INDEX IF NOT EXISTS idx_game_players_tee_order "
            "ON game_players(game_id, tee_order)"
        ))
    ...
    return {
        "message": "tee_order migration completed",
        "changes": changes
    }
