...
#### Backend Deployment (Cloud Run)
```yaml
Service: wolf-goat-pig-api
Region: us-central1
Image: Artifact Registry (built by Cloud Build)
...
Routing: firebase.json SPA rewrite
Production URL: https://seventh-country-232522.web.app
```

#### Database (Cloud SQL)
```yaml
Engine: PostgreSQL 17
Instance: wgp-postgres
Region: us-central1
Tier: db-f1-micro
Connection: Cloud SQL connector from Cloud Run
```
...
```bash
# Backend (Cloud Run; secrets injected from Secret Manager)
...
1. **Code Push**: Developer pushes to GitHub `main`
2. **Gate**: frontend/backend GitHub Actions run
3. **Backend Build**: Cloud Build publishes and deploys the Cloud Run image
4. **Frontend Build**: GitHub Actions builds and deploys Firebase Hosting
5. **Health Checks**: Cloud Monitoring verifies the production services
...
- **CDN Delivery**: Global asset distribution via Firebase Hosting
...
- **Backend**: Cloud Run request metrics and Cloud Logging
- **Frontend**: Firebase Hosting metrics and browser diagnostics
