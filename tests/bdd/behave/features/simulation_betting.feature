Feature: Simulation betting mechanics
  ...
  Scenario: Seed-state rejects requests without admin header
  ...
  Scenario: Redouble attempts stay capped once wagering is closed
    Given the simulation is at hole 9
    And the betting state is
      | current_wager   | 8    |
      | doubled         | true |
  ...
  Scenario: Carry-over clears once wager returns to base
  ...
  Scenario: Locking wagering freezes ping pong progression
    Given the simulation is at hole 3
  ...
  Scenario: Wagering reopens automatically on a new hole
    Given the simulation is at hole 11
    And wagering is set to closed
