...
def seed_state(client: TestClient, seed_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    response = client.post("/simulation/test/seed-state", json=seed_data, headers=ADMIN_HEADERS)
...
def make_simulation_request(
    ...
    *,
    ...
    if headers:
        final_headers = {key: "" if value is None else str(value) for key, value in headers.items()}

    needs_admin_header = path.startswith("/simulation/state") or path.startswith("/simulation/test/seed-state")
    if needs_admin_header and not omit_admin_header:
        if final_headers is None:
            final_headers = dict(ADMIN_HEADERS)
        else:
            for key, value in ADMIN_HEADERS.items():
                final_headers.setdefault(key, value)

    if final_headers is not None and not final_headers:
        final_headers = None

    if method_upper == "GET":
        response = client.get(path, headers=final_headers)
    elif method_upper == "POST":
        response = client.post(path, json=payload or {}, headers=final_headers)
    elif method_upper == "PUT":
        response = client.put(path, json=payload or {}, headers=final_headers)
    elif method_upper == "DELETE":
        response = client.delete(path, headers=final_headers)
    else:
        response = client.request(method_upper, path, json=payload or {}, headers=final_headers)
...
def parse_table_to_dict(table, key_column: int = 0, value_column: int = 1) -> Dict[str, str]:
    ...
    Supports tables specified with or without an explicit header row. If the
    first row is interpreted by Behave as headings but doesn't use conventional
    labels (like ``field``/``value``), the headings are treated as the first
    data row.
    """
    ...
    if table.headings:
        ...
        conventional = {"field", "value", "key", "path"}
        if not any(h in conventional for h in lower_headings):
