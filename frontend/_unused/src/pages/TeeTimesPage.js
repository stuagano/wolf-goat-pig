...
import BookingModal from '../components/foretees/BookingModal';
...
const TeeTimesPage = () => {
  ...
  // Auto-dismiss booking result toast
  useEffect(() => {
    if (bookingResult) {
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '20px 16px' }}>
      <h1 style={{ color: theme.colors.primary, marginBottom: 8 }}>Tee Times</h1>
      ...
      {/* My Bookings */}
      {bookings.length > 0 && (
        <div style={{ ...theme.cardStyle, marginBottom: 24, padding: 16 }}>
          <h3 style={{ color: theme.colors.primary, margin: '0 0 12px' }}>My Upcoming Tee Times</h3>
          {bookings.map((b, i) => (
            <div
              key={i}
              style={{
                ...
                padding: '8px 0',
                borderBottom: i < bookings.length - 1 ? `1px solid ${theme.colors.border || '#e5e7eb'}` : 'none',
                color: theme.colors.textPrimary,
              }}
            >
              <span>{b.date}</span>
              <span style={{ fontWeight: 600 }}>{b.time}</span>
              <span style={{ color: theme.colors.textSecondary }}>{b.course}</span>
            </div>
          ))}
        </div>
      )}
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            {slot.players.length > 0 ? (
              ...
              <div style={{ color: theme.colors.textSecondary, fontStyle: 'italic', fontSize: 14 }}>
                Open
              </div>
            )}
          </div>
