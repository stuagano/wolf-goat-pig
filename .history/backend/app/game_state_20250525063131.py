...
class GameState:
    ...
    def get_player_strokes(self):
        ...
        for player in self.players:
            ...
            if hcap > n_holes:
                extra = hcap - n_holes
                # Find holes with lowest stroke index (hardest holes)
                hardest = sorted(range(n_holes), key=lambda i: self.hole_stroke_indexes[i])[:extra]
                for i in hardest:
                    strokes[i+1] += 1
            # Example: for half-strokes, you could use 0.5 instead of 1
            # (customize as needed)
            result[pid] = strokes
        return result
